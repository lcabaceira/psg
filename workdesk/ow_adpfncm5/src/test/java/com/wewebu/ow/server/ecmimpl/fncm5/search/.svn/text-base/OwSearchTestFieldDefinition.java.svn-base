...
public class OwSearchTestFieldDefinition implements OwFieldDefinition
{
    ...
    private static final String VALUE_TYPE_BOOLEAN = "B";
    ...
    protected Object[] convertValues(String type_p, List<String> stringValues_p) throws OwInvalidOperationException, ParseException
    {
        ...
        else if (type_p.equals(VALUE_TYPE_BOOLEAN))
        {
            valuesArray = new Boolean[stringValues_p.size()];
        }
        else
        {
    ...
    public static Date toDate(String date_p)
    {
        XMLGregorianCalendar xmlGregorianCalendar = XMLGregorianCalendarImpl.parse(date_p);
        Calendar calendar = toCalendar(xmlGregorianCalendar);
        Date objectValue = calendar.getTime();

        return objectValue;
    }

    public static Calendar toCalendar(XMLGregorianCalendar calendar_p)
    {
        Calendar calendar = Calendar.getInstance();

        TimeZone timeZone = calendar_p.getTimeZone(DatatypeConstants.FIELD_UNDEFINED);
        calendar.setTimeZone(timeZone);

        calendar.set(Calendar.YEAR, calendar_p.getYear());
        calendar.set(Calendar.MONTH, calendar_p.getMonth() - 1);
        calendar.set(Calendar.DAY_OF_MONTH, calendar_p.getDay());
        calendar.set(Calendar.HOUR_OF_DAY, calendar_p.getHour());
        calendar.set(Calendar.MINUTE, calendar_p.getMinute());
        calendar.set(Calendar.SECOND, calendar_p.getSecond());
        calendar.set(Calendar.MILLISECOND, calendar_p.getMillisecond());
