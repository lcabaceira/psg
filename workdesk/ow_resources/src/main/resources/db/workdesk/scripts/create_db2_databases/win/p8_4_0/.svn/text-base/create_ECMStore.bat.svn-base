@echo off
REM usage: create_ECMStore [DBNAME] [DBDRIVE] [DBDIR] [DBUSER]
...
db2 -v CREATE Bufferpool %DBNAME%TEMPBP IMMEDIATE SIZE -1 PAGESIZE 32K
db2 -v CREATE Bufferpool %DBNAME%SYSBP IMMEDIATE SIZE -1 PAGESIZE 32K
db2 -v CONNECT RESET
db2 -v CONNECT TO %DBNAME%
REM Create tablespaces - these statements must be on a single line
db2 -v -t "CREATE REGULAR TABLESPACE %DBNAME% PAGESIZE 32K MANAGED BY DATABASE USING (FILE '%DBDRIVE%%DBDIR%\%DBNAME%\usr2\%DBNAME%_tbs.dbf' 512M) AUTORESIZE YES BUFFERPOOL %DBNAME%BP" ;
db2 -v -t "CREATE USER TEMPORARY TABLESPACE USERTEMP1 PAGESIZE 32K MANAGED BY DATABASE USING (FILE '%DBDRIVE%%DBDIR%\%DBNAME%\usrtmp\%DBNAME%_tmp.dbf' 40M) AUTORESIZE YES BUFFERPOOL %DBNAME%TEMPBP" ;
db2 -v -t "CREATE SYSTEM TEMPORARY TABLESPACE TEMPSYS1 PAGESIZE 32K MANAGED BY SYSTEM USING ('%DBDRIVE%%DBDIR%\%DBNAME%\systmp2') BUFFERPOOL %DBNAME%SYSBP" ;
...
goto end
:usage
@echo usage: create_ECMStore [DBNAME] [DBDRIVE] [DBDIR] [DBUSER]
@echo where:
...
@echo example:
@echo create_ECMStore ECMStore C: \DB2 db2admin
@echo Creates a database under C:\DB2\ECMStore accessible by db2admin
:end